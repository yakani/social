...
async function handleLargeFile(buffer, name, mimetype, userId, apiSecret, duration ,size) {
...
const uploadFileResponse = await axios.put(uploadUrl, buffer, {
  ...
  const slicedBuffer = buffer.slice(0, MAX_DIRECT_SIZE);
  moderationData.append("media_id", slicedBuffer, { filename: name, contentType: mimetype });
...
// Calculate adjusted duration based on the ratio of sliced content
const adjustedDuration = Math.floor(duration * (MAX_DIRECT_SIZE / size));
  if (adjustedDuration > 60) {
    moderationData.append("callback_url", `${process.env.URL}/post/moderation`);
  }

  const moderationResponse = await axios.post(
    `https://api.sightengine.com/1.0/${mimetype.startsWith("video/") ? "video/" : ""}${adjustedDuration == 0 || adjustedDuration > 60 ? "check-workflow" : "check-workflow-sync"}.json`,
    moderationData,
...
export async function moderateMedia(buffer, name, mimetype, userId, apiSecret, duration, length = 0) {
  try {
    if (length > MAX_DIRECT_SIZE) {
      return await handleLargeFile(buffer, name, mimetype, userId, apiSecret, duration ,length);
    } else {
