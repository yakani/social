...
const Accountpage = () => {
  ...
  useEffect(() => {
    setgoal( editMode ? saves[0].saves : postuser);
    getPeoplefollowing();
    getPeoplefollowers();
  }, [postuser ,saves]);
...
useEffect(()=>{
  setgoal(!editMode ? postuser : saves[0].saves);
},[editMode]);
  const handleImageUpload = async (e) => {
    const file = e.target.files[0];
    if (!file) return;
    if (file.size > 10 * 1024 * 1024)
      return toast.error("to large must be less than 10mb");
    ...
    <div className=" flex justify-center flex-col bg-neutral min-h-screen">
      ...
      <div className="flex justify-center m-1">
        <label htmlFor="my_modal_2">
          <p className="btn  btn-outline btn-warning m-1">{`${follower.length}${
            follower.length / 1000 >= 1 ? "k" : ""
          } Followers`}</p>
        </label>
        ...
        <div className="modal" role="dialog">
          <div className="modal-box">
            {follower.length == 0 ? (
              ...
              <List title="Followers" items={follower} />
            )}
          </div>
        ...
        <label htmlFor="my_modal_1">
          <p className="btn  btn-outline btn-warning m-1">
            {" "}
            {`${follow.length}${
              follow.length / 1000 >= 1 ? "k" : ""
            }  Following`}
          </p>
        </label>
        ...
        <div className="modal" role="dialog">
          <div className="modal-box">
            {follow.length == 0 ? (
              <p className="text-xl text-center "> Following No body </p>
            ) : (
              <List title="Following" items={follow} />
            )}
          </div>
        ...
        <div className="flex border-b border-warning px-4 justify-between">
          <a
            className={`flex flex-col items-center justify-center border-b-[3px] ${
              editMode ? "border-b-transparent" : "border-b-warning"
            }   pb-[13px] pt-4 flex-1`}
            href="#"
      ...
      <div className="grid grid-cols-[repeat(auto-fit,minmax(158px,1fr))] gap-3 p-4 ">
        {isloadingpostuser ? (
          skeletonContacts.map((t, id) => <Presentskeleton key={id} />)
        ) : goal.length == 0 ? ( 
          <div className="flex flex-col justify-center items-center">
            { editMode ? <p className="text-center  m-auto p-auto text-lg non-italic">No video saves yet</p> :<>
            <Search className="size-30 text-warning m-2" />
            <button
          ...
          goal.map((t) => (
            ...
            >
              <Present post={t} admin={true} func={deletepost} />
            </div>
          ))
        )}
      </div>
