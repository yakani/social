...
export const Userstore = create((set, get) => ({
  ...
  getsavesvideo:async()=>{
    try {
      const res =  await axiosInstance.get('user/saves');
      set({saves:res.data});
    } catch (error) {
      console.log(error);
    }
  ...
  getobservation: async ()=>{
    try {
      const res = await axiosInstance.get('user/view');
      set({observation:res.data});
    } catch (error) {
      console.log(error);
    }
  },
  ...
  Loadsavevisitor:async(id)=>{
    set({isloadingsaves:true});
    try {
      const res = await axiosInstance.get('user/saves/visitor/'+id);
      set({savesvisitors:res.data});
    } catch (error) {
      console.log(error);
    }finally{
